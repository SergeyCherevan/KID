...
todos:
  - id: remove-mouse-init
    content: Удалить `Mouse.Init(canvas)` из `KID/Services/CodeExecution/Contexts/CanvasGraphicsContext.cs`.
    status: pending
  - id: delete-mouse-library
    content: Удалить папку `KID/KIDLibrary/Mouse/` целиком (все файлы Mouse API).
    status: pending
  - id: delete-mouse-template
    content: Удалить `KID/ProjectTemplates/MouseTest.cs`.
    status: pending
  - id: update-docs
...
- **Выход**:
  - Удалены файлы `KID/KIDLibrary/Mouse/*`.
...
- **KIDLibrary layer**: удаляется `Mouse API` (папка `KID/KIDLibrary/Mouse/`).
- **Code Execution / Contexts**: `CanvasGraphicsContext.Init()` сейчас вызывает `Mouse.Init(canvas)`.
- **ProjectTemplates**: есть шаблон `KID/ProjectTemplates/MouseTest.cs`.
...
- Удалить вызов `Mouse.Init(canvas)` в:
  - `KID/Services/CodeExecution/Contexts/CanvasGraphicsContext.cs`
...
- Удалить папку целиком: `KID/KIDLibrary/Mouse/` (все `*.cs`).
  - Ожидаемые файлы (по текущей документации/структуре):
    - `KID/KIDLibrary/Mouse/Mouse.System.cs`
    - `KID/KIDLibrary/Mouse/Mouse.Position.cs`
    - `KID/KIDLibrary/Mouse/Mouse.Click.cs`
    - `KID/KIDLibrary/Mouse/Mouse.Events.cs`
    - `KID/KIDLibrary/Mouse/Mouse.State.cs`
    - `KID/KIDLibrary/Mouse/ClickStatus.cs`
    - `KID/KIDLibrary/Mouse/MouseClickInfo.cs`
    - `KID/KIDLibrary/Mouse/PressButtonStatus.cs`
    - `KID/KIDLibrary/Mouse/CursorInfo.cs`

3.2. **Удаление интеграции Mouse из инициализации**
- Обновить `KID/Services/CodeExecution/Contexts/CanvasGraphicsContext.cs`:
  - удалить строку `Mouse.Init(canvas);`

3.3. **Удаление шаблонов**
- Удалить `KID/ProjectTemplates/MouseTest.cs`
...
- Обновить ссылки/разделы:
  ...
  - `docs/ARCHITECTURE.md` (убрать Mouse из описания KIDLibrary/потоков данных/расширяемости)
...
4.2. Удалить `KID/KIDLibrary/Mouse/*`.
4.3. Удалить `KID/ProjectTemplates/MouseTest.cs`.
