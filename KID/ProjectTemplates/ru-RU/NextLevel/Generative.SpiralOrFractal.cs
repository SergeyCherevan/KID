...
// Генеративный рисунок (спираль).
// Чему учит: циклы, математика (sin/cos), параметры, эксперименты.
//
// Управление:
// - Space = перерисовать
// - Up/Down = изменить количество точек
// - Left/Right = изменить «закрученность»
// - Esc = выход

int points = 1200;
double turns = 10; // сколько оборотов
...
void Draw()
{
    ...
    // Палитра: меняем цвет каждые N шагов
    ...
    Graphics.Text(10, 10, $"Генеративная спираль  points={points}  turns={turns}");
    Graphics.Text(10, 30, "Space=перерисовать  Up/Down=точки  Left/Right=обороты  Esc=выход");
}
