...
## Жизненный цикл запуска и Stop

IDE публикует единый `ExecutionEnvironment` с execution id/token, а графический контекст подключает к нему Dispatcher scope; пользователь не вызывает Init. Scope получает identity и token из environment. Вызовы через Graphics/DispatcherManager проверяют Stop перед постановкой команды. Ожидающий результат пользовательский поток освобождается по отмене, даже если UI занят. Pending-команды отменяются или пропускаются; cancellation exception не выходит из queued callback в WPF.

При штатном завершении принятые команды дополняют рисунок до завершения cleanup. После Stop новые команды не принимаются. Уже выполняющаяся команда выходит кооперативно: прервать произвольную WPF/native-функцию насильно невозможно. Cleanup ожидает её фактического окончания, поэтому следующий Run не пересекается с ней. Ошибка fire-and-forget команды учитывается как ошибка cleanup.

CanvasGraphicsContext.DisposeAsync идемпотентно ожидает очередь и без session token сбрасывает статическую ссылку Canvas, заливку/обводку в Black и шрифт в Arial 20. Готовый рисунок остаётся видимым; Canvas.Children очищается существующим сценарием следующего Run. Размеры Canvas остаются состоянием представления IDE, а не сбрасываемыми настройками пера.

Гарантии относятся к библиотечным операциям. Прямой доступ к возвращаемым WPF-объектам, собственные потоки и обработчики пользователя не изолированы. Сохранённые UI-объекты с пользовательскими делегатами могут удерживать сборку. `CanvasGraphicsContext.DisposeAsync` также закрывает и ожидает per-run Keyboard/Mouse/Music scopes до освобождения Graphics.
...
Ниже — краткое описание того, какие **паттерны проектирования** и **архитектурные особенности** реализованы в модуле `KID.Library/Graphics`.
...
- **Граница слоя “KIDLibrary API для пользовательского кода”**
  - `Graphics` находится в `KID.Library` и является библиотечным API, предназначенным для вызова из пользовательского кода, а не частью MVVM/сервисного слоя.

- **Жизненный цикл через контекст выполнения**
  - `CodeExecutionService` публикует `ExecutionEnvironment`, `CanvasGraphicsContext` подключает Dispatcher capability, после чего вызывает внутренний `Graphics.Init(Canvas, scope)` и инициализирует `Mouse`/`Music`.

- **Потокобезопасность — базовое правило**
  - Любой доступ к WPF-объектам выполняется через `DispatcherManager`. `void`-операции ставятся через отслеживаемый `DispatcherOperation`; `Func<T>` ожидает его результат с session token и освобождает worker после Stop даже при занятом UI.
