...
IDE открывает внутренний Dispatcher scope с execution id/token; пользователь не вызывает Init. Вызовы через Graphics/DispatcherManager проверяют Stop перед постановкой команды. Ожидающий результат пользовательский поток освобождается по отмене, даже если UI занят. Pending-команды отменяются или пропускаются; cancellation exception не выходит из queued callback в WPF.
