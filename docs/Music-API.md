...
Воспроизводит последовательность звуков без пауз между ними. Параметры должны идти парами: частота, длительность, частота, длительность, ...
...
**Параметры:**
- `frequencyAndDuration` - пары значений: частота, длительность, частота, длительность, ...
...
// Мелодия с паузой
Music.Sound(262, 500, 294, 500, 330, 500, 0, 250, 262, 1000);

// Можно передать массив
double[] melody = { 262, 500, 294, 500, 330, 500 };
Music.Sound(melody);
```
...
Воспроизводит несколько дорожек одновременно с микшированием. Двумерный массив: первый индекс - номер дорожки, второй - пары частота/длительность.
...
**Параметры:**
- `polyphonicSounds` - двумерный массив: [дорожка][частота, длительность, частота, длительность, ...]
...
double[,] threeVoices = {
    { 262, 2000 },  // Голос 1: До
    { 330, 2000 },  // Голос 2: Ми
    { 392, 2000 }   // Голос 3: Соль
};
Music.Sound(threeVoices); // Аккорд До-Ми-Соль
```
...
Music.Sound(262, 500, 294, 500, 330, 500, 262, 1000);
```
...
2. **Потокобезопасность**: Все операции автоматически выполняются в UI потоке, что обеспечивает безопасность.
...
for (int i = 0; i < 100; i++)
{
    Music.Sound(440 + i * 10, 100); // Каждая нота проверяет StopManager
}
```
