...
Host создаёт отдельный `MusicExecutionScope` при инициализации каждого execution-контекста.
Пользовательскому коду не требуется и недоступна ручная инициализация. Новые звуки принимаются
только пока scope текущего Run открыт.
...
2. **Потокобезопасность**: В реализации `Music` нет прямой работы с WPF UI, поэтому `DispatcherManager` не используется. Активные звуки, задачи и временные файлы хранятся в отдельном `MusicExecutionScope` текущего запуска; locks защищают только короткие переходы состояния и не удерживаются во время `await` или NAudio `Dispose`.

3. **Отмена выполнения**: Music получает token из общего `ExecutionEnvironment`. Он проверяется в обходах нот/дорожек и передаётся в playback/fade delay, HTTP и файловые async-операции, поэтому Stop прерывает как блокирующий `Sound`, так и фоновый `SoundPlay`.

4. **Управление ресурсами**: `SoundStop`, `SoundPlayer.Dispose()` и `SoundPlayerOFF` позволяют остановить звук раньше. Независимо от них host всегда вызывает идемпотентный `ShutdownAsync`: запрещает новые регистрации, останавливает output, отменяет и ожидает фоновые задачи, освобождает NAudio/file resources и удаляет временные URL-файлы.
...
Публичный `StopManager` и внутренний Music scope используют token одного `ExecutionEnvironment`. Пользовательский цикл по-прежнему может явно вызвать `StopManager.StopIfButtonPressed()`, а внутри Music точный token исходного scope проверяется повторно, чтобы поздняя операция старого запуска не приняла identity нового.
...
Ниже — краткий разбор того, какие архитектурные приёмы и паттерны заложены в реализации `Music` (в коде `KID.Library/Music/*.cs`).
...
- **Adapter**: `IMusicRuntime`, `IMusicOutput` и `IMusicFileSource` изолируют NAudio, HTTP и файловую систему от lifecycle-координатора.
- **Per-run Registry + Handle API**: реестр живёт в `MusicExecutionScope`; `soundId` удобен пользователю, но ownership подтверждается ссылками на scope и playback, а не одним числом.
- **Dispose / RAII**: `MusicPlayback` является единым владельцем output/source/linked CTS; host ожидает его задачи до освобождения ресурсов, а ошибки отдельных Stop/Dispose агрегируются после попытки очистить всё.

### Архитектурные особенности

- **Слой `KIDLibrary`**: `Music` — часть библиотеки, доступной пользовательскому коду (аналогично `Graphics`, `Mouse`, `StopManager`).
- **Кооперативная отмена выполнения**: длительные операции регулярно проверяют session token исходного Music scope; поддерживающие cancellation API получают его напрямую.
- **Синхронизация через `lock`**:
  - глобальная громкость `Music.Volume` защищена блокировкой;
  - реестр активных звуков, задач и временных файлов защищён блокировкой своего per-run scope;
...
- **Полифония как streaming composition**: дорожки собираются из конечных sample providers, дополняются тишиной до общей длительности и подаются в `MixingSampleProvider` без собственного бесконечного массива сэмплов.
...
проверяют все системные кодеки/драйверы. Реальный звук относится к отдельному smoke/visual слою.
Если сторонний audio/native вызов не возвращает управление, in-process host не обещает его
