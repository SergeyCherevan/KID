...
- **Категории**:
  - `Gold/` — быстрые «золотые» примеры (рисование, мини‑игры, музыка)
  - `NextLevel/` — примеры со структурой (сцены, частицы, генеративная графика)
  - `CheatSheets/` — шаблоны‑памятки (игровой цикл, безопасный ввод)

### Как открыть шаблон

- **Через UI**: `Файл → Открыть` и выбрать нужный файл из `ProjectTemplates/...`
- **Через настройки**: в `AppData/KID/settings.json` указать путь в `TemplateName`, например `ProjectTemplates/ru-RU/Gold/MousePainter.cs`
