...
## Принадлежность запуску и отмена

Sprite создаётся внутри активного Dispatcher scope и сохраняет его вместе со ссылкой на исходный `ExecutionEnvironment`. Старый Sprite не может автоматически подключиться к новому запуску: его операции завершаются отменой или ошибкой завершённого scope.

Обходы элементов при создании, показе/скрытии, перемещении и collision вызывают единую проверку исходного environment на каждой итерации, включая внутренний цикл столкновений. Это кооперативная отмена: уже сделанные изменения не откатываются. Пока UI занят обходом, нажатие кнопки Stop может ждать обработки UI-события; проверки реагируют на уже отменённый token. Произвольные пользовательские getter и native/WPF-вызовы принудительно не прерываются.
